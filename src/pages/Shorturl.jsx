...
export default function UrlDashboard() {
  ...
  const fetchUserHistory = async () => {
    ...
    setError("");
    try {
      const res = await axios.get("http://:35.202.71.133:3000/user/history", {
  ...
  const fetchUrlHistory = async (shorturl_id) => {
    setLoading(true);
    try {
      const res = await axios.get(`http://35.202.71.133:3000/history/${shorturl_id}`, {
        withCredentials: true,
      });
  ...
  const deleteUrl = async (shorturl_id) => {
    ...
    setLoading(true);
    try {
      await axios.delete(`http://35.202.71.133:3000/${shorturl_id}`, {
        withCredentials: true,
      });
